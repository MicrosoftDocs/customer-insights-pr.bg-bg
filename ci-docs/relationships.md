...
- Нередактивните системни релации се създават от системата като част от процеса на обединяване на данни
- Нередактивните наследствени релации се създават автоматично от въвеждането на източници на данни
- Редактивните персонализирани релации се създават и конфигурират от потребителите
...
3. В екрана **Нова релация** посочете следната информация:
   ...
   - **Изходна кардиналност**: Кардиналност на обекта източник. Кардиналността описва броя на възможните елементи в даден набор. Винаги се отнася до целевата кардиналност. Можете да избирате между **Едно** и **Много**. Поддържат се само релации „много към един” и „един към един”.  
   ...
   - **Целева кардиналност**: Кардиналност на записите на целевия обект.
   - **Поле** за изходен ключ: Поле за чужд ключ в обекта източник. Пример: SupportCase използва **CaseID** като чуждо ключ поле.
   - **Поле** за целеви ключ: Ключово поле на целевия обект. Пример: Клиентът използва **CustomerID** като ключово поле.
...
Среди, които са конфигурирани да използват бизнес акаунти, тъй като основната целева аудитория може да конфигурира йерархиите на профила за свързани бизнес акаунти. Например компания, която има отделни бизнес единици.
...
1. В екрана **Йерархия на акаунта**, предоставете име за йерархията. Системата създава име за изходния обект, но можете да го промените.
1. Изберете обекта, който съдържа йерархията на вашия акаунт. Обикновено се намира в същия обект, който съдържа сметките.
1. Изберете UID **на** профила и **Родителски UID** от избрания обект.
1. Изберете **Запиши**, за да финализирате йерархията на профила.

## <a name="manage-existing-relationships"></a>Управление на съществуващи релации

Отидете на **страницата Релации,** за да прегледате всички релации, които са създадени, техния обект източник, целевия обект и кардиналността.
...
Използвайте **опциите "Филтриране по** " или **"Търсене на** релации", за да намирате определена релация. За да видите мрежова диаграма на съществуващите релации и тяхната кардиналност, изберете [**Визуализатор**](#explore-the-relationship-visualizer).

Изберете релация, за да прегледате наличните действия:
...
За да персонализирате изгледа, можете да промените позицията на полетата, като ги плъзнете върху платното. Другите опции включват: 
...
Пътят на отношенията описва обектите, които са свързани с връзки между обект източник и целеви обект. Използва се при създаване на сегмент или мярка, която включва обекти, различни от обекта на унифицирания профил, и има няколко опции за достигане до унифицирания обект на профил. Различните пътища на релациите могат да дадат различни резултати.
