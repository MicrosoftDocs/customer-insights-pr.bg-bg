...
- **Оперативни събития**
  - **WorkflowEvent** - Работният поток позволява на един да настроите [Източници на данни,](data-sources.md), [унифициране](data-unification.md) и [обогатяване](enrichment-hub.md) и накрая [експортиране](export-destinations.md) на данни в други системи. Всички тези стъпки могат да се направят индивидуално (например да предизвикат единен износ) или да бъдат оркестрирани (напр. опресняване на данните от източници на данни, които задействат процеса на обединение, който ще изтегли допълнителни обогатявания и след като бъде направен износ на данните в друга система). За повече подробности вижте работния [потокСъбитие схема](#workflow-event-schema).
  - **APIEvent** - всички API повиквания към екземпляра на клиентите да Dynamics 365 Customer Insights. За повече подробности вижте [Схемата на APIEvent](#api-event-schema).
...
- Имате активен абонамент за [Azure](https://azure.microsoft.com/pricing/purchase-options/pay-as-you-go/).
- Имате [администраторски](permissions.md#administrator) разрешения в "Аналитични данни за клиенти".
- Имате ролята на администратор на **сътрудник** и потребителски достъп на местоназначение ресурс на **Azure**. Ресурсът може да бъде акаунт за хранилище в Azure, център за събития в Azure или работна област на Azure Log Analytics. За повече информация вижте Добавяне или премахване на присвоявания на роля на [Azure с помощта на портала на Azure](/azure/role-based-access-control/role-assignments-portal).
- [Изпълнени са изискванията за местоназначение за хранилище в Azure, център за събития в Azure или Анализ на регистрационния файл на](/azure/azure-monitor/platform/diagnostic-settings#destination-requirements) Azure.
...
1. Изберете **наемателя на абонамента за** Azure с ресурса местоназначение и изберете **влезте в**.
...
В момента [се поддържат API събития и събития на](apis.md) работния поток и се прилагат следните категории и схеми.
...
- **Събития за проверка : API събития за проследяване на промените в**[конфигурацията](#api-event-schema) на услугата. `POST|PUT|DELETE|PATCH` операциите влизат в тази категория.
- **Оперативни събития** : [API събития](#api-event-schema) или събития на работен [поток,](#workflow-event-schema) генерирани при използване на услугата.  Например заявки `GET` или събитията за изпълнение на работен поток.
...
API събития и събития на работен поток имат обща структура и подробности, където те се различават, вижте [API събитие схема](#api-event-schema) или [работен поток събитие схема](#workflow-event-schema).
...
| `category`        | String    | Необходимо          | Регистрационна категория на събитието. Или `Operational``Audit` или. Всички POST/PUT/PATCH/DELETE HTTP Заявки са маркирани с `Audit`, всичко останало с`Operational` | `2020-09-08T09:48:14.8050869Z`                                                                                                                                           |
...
| `identity`        | String    | Незадължителен          | JSON обект, описващ самоличността на потребителя или приложението, което е извършило операцията.       | Вижте [Секция](#identity-schema) "Самоличност".     |  |
...
`identity` Обектът JSON има следната структура
...
| Поле                         | Описание                                                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `Authorization.UserRole`      | Присвоена роля за потребителя или приложението. За повече информация вижте [потребителските разрешения](permissions.md).                                     |
...
| `Claims`                      | Претенции на потребителя или приложението JSON уеб маркер (JWT). Свойствата на претенцията варират за организация и Azure Active Directory конфигурацията. |
...
Работният поток съдържа няколко стъпки. [Най-много източници на данни](data-sources.md), [унифициране](data-unification.md), [обогатяване](enrichment-hub.md), и [експортиране](export-destinations.md) на данни. Всички тези стъпки могат да се изпълняват индивидуално или оркестрирани със следните процеси. 
...
| `category`      | String    | Необходимо          | Регистрационна категория на събитието. Винаги за събития от `Operational` работния поток                                                                                           | `Operational`                                                                                                                                                            | 
...
| `properties.operationType`                   | Да      | Да  | Идентификатор на операцията вижте [Типове операции]. (#operation типове)                                                                                                                                                                                       |
| `properties.tasksCount`                      | Да      | No   | Само работен поток. Брой задачи задейства работния поток.                                                                                                                                                                                                       |
| `properties.submittedBy`                     | Да      | No   | По избор. Само за събития от работния поток. Azure Active Directory [ObjectId на](/azure/marketplace/find-tenant-object-id#find-user-object-id) потребителя, който е задействал работния поток, вижте също `properties.workflowSubmissionKind`.                                   |
...
| `properties.additionalInfo.Kind`             | No       | Да  | По избор. Само за `Export` ОперацияТип. Идентифицира вида на експортирането. За повече информация вижте [преглед на дестинациите за износ](export-destinations.md).                                                                                          |
| `properties.additionalInfo.AffectedEntities` | No       | Да  | По избор. Само за `Export` ОперацияТип. Съдържа списък с конфигурирани обекти в експортирането.                                                                                                                                                            |
| `properties.additionalInfo.MessageCode`      | No       | Да  | По избор. Само за `Export` ОперацияТип. Подробно съобщение за експортирането.                                                                                                                                                                                 |
| `properties.additionalInfo.entityCount`      | No       | Да  | По избор. Само за `Segmentation` ОперацияТип. Показва общите номера на членовете, които сегментът има.                                                                                                                                                    |
