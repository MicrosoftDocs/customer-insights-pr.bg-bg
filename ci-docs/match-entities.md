...
1. Отидете на **Данни** > **Обединяване** > **Съвпадение** и изберете **Задаване на ред**, за да започне фазата на мача.
1. Изберете **Поръчка на обект**. Например изберете **eCommerce:eCommerceКонтакти** като основен обект и **LoyaltyScheme:loyCustomers** като втори обект. 
1. За да имате всеки запис в обекта като уникален клиент и съответстващ на всеки следващ обект, изберете **Включване на всички**.
1. Изберете **Готово**. 

След като посочите реда на съвпадението, дефинираните двойки на съвпадението се показват в **секцията**"Съвпадащи записи" в **DataUnifyMatch** > **·** > **·**. Ключовите показатели са празни, докато процесът на съвпадение завърши.
...
Първичният обект *eCommerce:eCommerceContacts* се съчетава със следващия обект *LoyaltyScheme:loyCustomers*. Наборът от данни, който е резултат от първата стъпка на съвпадение, е съчетан със следния обект, ако имате повече от два обекта.
...
1. Изберете **Добавяне на правило** под обект в секцията **Съвместяване на подробни данни** за записи, за да определите правилата за съвпадение.
...
1. В екрана **Създайте правило**, конфигурирайте условията за правилото.
   ...
   - **Прецизност**: Задайте нивото на точност, за да кандидатствате за това състояние. 
     - **Основен**: Изберете от *Ниска*, *Средна*, *Висока* и *Точно*. Изберете **Точно**, за да съответствате само на записи, които съответстват на 100 процента. Изберете едно от другите нива за съвпадение на записи, които не са 100 процента идентични.
...
Дедъпликирани записи се използват в процеса на съпоставяне между обект. Дедупликацията се случва на отделни обекти и може да бъде конфигурирана за всеки обект, използван в двойки на съвпадение.
...
1. В секцията **Дедъпликирани записи подробности** изберете **Задаване на обекти**. В случай, че правилата за дедупликация вече са създадени, изберете **редактиране**.
...
1. След като [изпълните процеса](#run-the-match-process) на съвпадение, ще видите статистиките за дедупликация в плочките с ключови показатели.
...
Ако сте обогатявали обекти на ниво източник на данни, изберете ги, преди да изпълните процеса на съвпадение. Обогатените обекти могат да подобрят резултатите ви от обединението. За повече информация вижте [Обогатяване за източници на](data-sources-enrichment.md) данни. 

Обогатеният обект съдържа оригиналните източник на данни полета и обогатените полета. Така че, ако изберете да работите с обогатения обект, съществуващата конфигурация не се въздейства. Може обаче да се наложи да актуализирате правилата за съвпадение, за да използвате вместо това обогатените полета.

1. Отидете на **DataUnifyMatch** > **·** > **и** изберете **Използване на обогатени обекти** в горната част на страницата.

1. От екрана Използване на **обогатени обекти** изберете един или повече обогатени обекти.

1. Изберете **Готово**. Където и да се използва обектът източник (като поръчка за съвпадение или правила), той автоматично се променя на обогатения обект.
...
1. В екрана с **правила "Редактиране"** изберете **Добавяне на изключение**.
...
1. В **потребителския** екран отидете в раздела **Записи**.

1. Изберете опцията персонализирано съвпадение от **потребителски тип** отпадане и изберете **Изтегляне на шаблон**. Трябва ви отделен шаблон за всяка опция за съвпадение.
...
1. След като качите файловете и обектите са налични, изберете опцията **Персонализирано съвпадение** отново. Ще видите опции за указване на обектите, които искате да включите. Изберете задължителните обекти от падащото меню и изберете **Готово**.
...
1. Прилагането на персонализираното съвпадение зависи от опцията за съвпадение, която искате да използвате. 
   ...
   - За **съпоставяне** по избор на байпас **или** Псевдоним изберете **Редактиране** на съществуващо правило за съвпадение или създайте ново правило. В нормализирания отпадане изберете **опцията Персонализиран байпас** или **Псевдоним съпоставяне** и изберете **Готово**.
...
- Самофлацията не показва нормализираните данни в дедупликационните обекти. Въпреки това, той прилага нормализирането вътрешно по време на дедупликация. По дизайн е за всички нормализирания. 
- Ако настройката за семантичен тип е премахната във фаза **карта**, когато правило за съвпадение използва съпоставяне на Псевдоним или Персонализиран байпас, нормализирането няма да бъде приложено. Това се случва само ако изчистите семантичния тип след конфигуриране на нормализирането в правилото за съвпадение, защото семантичният тип ще бъде неизвестен.


## <a name="next-step"></a>Следваща стъпка

След завършване на процеса на съвпадение за поне една двойка съвпадение, продължете към стъпката [**Обединяване**](merge-entities.md).
