...
description: Дефинирайте дейностите за контакт с клиенти или бизнес и ги преглеждайте във времева линия в профилите на клиентите.
ms.date: 08/12/2022
...
## <a name="define-a-customer-activity"></a>Дефиниране на дейност на клиент
...
1. В стъпката "Релация **" изберете** Добавяне на **релация**, за да свържете данните за дейността си към това е съответния запис на клиент. Тази стъпка визуализира връзката между обектите.  
   ...
   - **Чужд ключ**: Чуждо поле във вашия обект на дейност, което ще се използва за установяване на релация с друг обект.
   ...
   - **Име** на релация: Ако вече съществува релация между този обект на дейност и избрания обект клиент източник, името на релацията ще бъде в режим само за четене. Ако такава връзка не съществува, ще се създаде нова връзка с името, което сте посочили в това поле.
...
## <a name="manage-existing-customer-activities"></a>Управление на съществуващите клиентски дейности
...
## <a name="define-a-contact-activity"></a>Дефиниране на дейност за контакт

За бизнес акаунти (B-to-B) използвайте *обект на ContactProfile*, за да заснемете дейности на контакти. Можете да видите в хронологията на дейността за даден акаунт кой контакт е отговарял за всяка дейност. Повечето стъпки следват конфигурацията за съпоставяне на активността на клиента.

   > [!NOTE]
   > За да се определи дейност на ниво контакт, *трябва да се създаде обект на ContactProfile*, или като унифициран [профил](data-unification-contacts.md) за контакт, или чрез [семантично съпоставяне](semantic-mappings.md#define-a-contactprofile-semantic-entity-mapping).
   >
   > Трябва да имате както AccountID **, така** и **ContactID** атрибути за всеки запис в рамките на данните за дейността си.
...
1. В стъпката **Релации** създайте непряка релация между данните си източник на дейност към акаунти, като използвате данните си за контакт като обект посредник. За повече информация вижте [пътищата за преки и косвени релации](relationships.md#relationship-paths).
...
1. След като настроите релации, изберете **Напред** и завършете конфигурацията си за съпоставяне на дейности. За подробни стъпки при създаването на дейност вижте [определяне на дейност](#define-a-customer-activity) на клиент.
